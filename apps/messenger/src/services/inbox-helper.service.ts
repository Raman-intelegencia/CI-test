import { Message, Profiles, Threads, UserProfileService, Users, loadLatestMessage,SelectedThreadHelperService } from "@amsconnect/shared";
import { Injectable } from "@angular/core";
import { BehaviorSubject } from "rxjs";
import { ThreadHelperService } from "../../../../libs/shared/src/lib/services/messenger-services/thread-helper.service";
...
export class InboxHelperService {
    ...
    constructor(private userProfileSvc: UserProfileService,
        ...
        public fetchThreadsAndProfiles(archive:boolean, checkBoxValue?:any): void {
            ...
            const archivevalue = this.threadHealperSvc.getArchiveCheckboxState()
            this.userProfileSvc
                .getMessageload_latest2(journalId, archivevalue)
                .subscribe((data: loadLatestMessage) => {
                    ...
                    // check if the journalId is greater than the existing journalId then update the observable with new journalID
    ...
    public handleMessageEntries(message: Message) {
        ...
        if (lastThread) {
            ...
            this.userProfileSvc
                ...
                .subscribe({
                    ...
                    if (olderThreadsData && olderThreadsData.threads.length > 0) {
                        ...
                        this.isLoadOlderThreadsDataFetched = false;
                            onComplete(true); // Call onComplete with true if data is received
                        } else {
                            this.isLoadOlderThreadsDataFetched = false;
                            onComplete(false); // Call onComplete with false if data is empty
                        }
                    },
                    error: (error) => {
                        ...
                        this.isLoadOlderThreadsDataFetched = false;
                        onComplete(false); // Call onComplete with false in case of error
                    }
                });
        }
    }
