...
// reversed and grouped
const GROUP_SENT = [SAMPLE_PAYLOAD.thread.messages[4], SAMPLE_PAYLOAD.thread.messages[3], SAMPLE_PAYLOAD.thread.messages[2]];
const PROFILE_SENT = SAMPLE_PAYLOAD.profiles[0];
const RECIPIENT = SAMPLE_PAYLOAD.user_id;

it('renders without crashing', () => {
  const div = document.createElement('div');
  ReactDOM.render(<MessageGroup type="sent" messages={GROUP_SENT} recipient={RECIPIENT} profile={PROFILE_SENT} user_thread_external_id={undefined} />, div);
  ReactDOM.unmountComponentAtNode(div);
});

it('type prop is applied as class modifier', () => {
  const div = document.createElement('div');
  ReactDOM.render(<MessageGroup type="abc123" messages={GROUP_SENT} recipient={RECIPIENT} profile={PROFILE_SENT} user_thread_external_id={undefined} />, div);

  const worked = div.querySelector('.messagegroup').classList.contains('messagegroup--abc123');
...
it('should have one <Profile />', () => {
  const wrapper = shallow(<MessageGroup type="abc123" messages={GROUP_SENT} recipient={RECIPIENT} profile={PROFILE_SENT} user_thread_external_id={undefined} />);
